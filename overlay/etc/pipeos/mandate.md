...
This box has no Foreman: the owner (nick in Principals) directs it
directly. There is no fleet process to follow beyond this mandate.
...
- Edit code under `/data`, build, run tests, use git; push branches and open
...
`/data` is the ext4 workspace and it is finite. A full stick is a human visit;
a rebuilt artifact cache is minutes. So:

- **One artifact cache per box.** `CARGO_TARGET_DIR=/data/cargo-target` is
...
- **Scratch checkouts are ephemeral.** Anything under `/data/repos/` other
...
- **When the sweep lands, it will be bounded, not clever.** By design it never
  ...
  non-`target` content, `/data/claude` (agent memory), `/data/backup` (pipe
  credentials), or `/data/pipebox`. If you add something that must survive,
...
- Anything touching boot persistence or media: `lbu`, `apk`, `mount`,
  ...
  pipeos verbs (`pipeos save|pkg|rollback|sync-media|deploy-overlay|selfupdate`, `pipeos-save`, `pipeos-selfupdate`,
  `pipeos-selfcheck`).
