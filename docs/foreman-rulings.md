...
| R22 | 2026-08-15 | Sam: OWNER WORK OUTRANKS BOX-GENERATED WORK. Claim order is (1) `owner`-labelled Ready, oldest first, (2) other Ready in lane, oldest first, (3) nothing — report and stop. A box-filed issue lands in Backlog and is unclaimable by any box until the Foreman promotes it to Ready; filing is not claiming. Exception: a defect blocking a held owner item is fixed inside that claim. Enforced in the mandate's claiming section (card-generated) and by kanban status | board thread 78 + mandate.md.tmpl |
