...
p14b = subprocess.run([sys.executable, "-c", "import sys; sys.path.insert(0, %r); import vault; r=[x for x in vault.list_() if x['name']=='jobs.a'][0]; print(r.get('shared'), vault.get('jobs.a')); print(vault.unset_shared('jobs.a','2222'), vault.unset_shared('jobs.a','2222')); print(vault.shareable('jobs.a'), vault.shareable('assistant_pass'), vault.shareable('stream_key_1'), vault.shareable('claude_token'), vault.shareable('x','bytes'))" % D],
                      capture_output=True, text=True, env=envp)
rc_l, out_l = vault("list")
check("14 set_shared notes a member on the entry; a re-set of the value keeps the note; the listing shows it and never the value; unset_shared forgets once; shareable = text and not one Machine's own (assistant_pass, stream keys) — the Claude token and jobs.* are",
      "'2222': 1700000000" in p14.stdout and p14b.stdout.startswith("{'2222': 1700000000} two") and "True False" in p14b.stdout
