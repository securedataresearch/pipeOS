...
modes = subprocess.run(GIT + ["-C", REPO, "ls-files", "-s", "overlay/etc/init.d", "overlay/usr/local/bin", "overlay/etc/local.d", "overlay/etc/periodic"],
...
# ── 17b. a box claimed through the wizard has an EMPTY NICK (a GENERIC box) and the
# provisioned marker; a template deploy must regenerate there too (pipeOS#313: zero
# read CRITICAL after two template deploys on 2026-09-19 because this step took an
# empty NICK for "unprovisioned")
c17b = case(fixture=REAL_FIXTURE, card=REAL_CARD.replace("NICK=box0", "NICK=").replace("NICK=\"box0\"", "NICK="))
write(os.path.join(c17b.root, "etc/pipeos/provisioned"), "")
g = subprocess.run(["sh", PBC, "generate", "--card", os.path.join(c17b.root, "etc/pipeos/card.conf"),
                    "--root", c17b.root, "--templates", TMPL_DIR], capture_output=True, text=True)
assert g.returncode == 0, "row 17b setup: generate failed: " + g.stderr
tp = os.path.join(c17b.repo, "overlay/usr/local/share/pipeos/card/mandate.md.tmpl")
write(tp, open(tp).read() + "\nAnother line a release adds.\n")
git(c17b.repo, "add", "-A"); git(c17b.repo, "commit", "-qm", "template change")
c17b.run()
check("17b a claimed box with an empty NICK (GENERIC, claimed through the wizard: the provisioned marker) has its card outputs regenerated by a template deploy too, and verify is clean after (pipeOS#313)",
      c17b.rc == 0 and "regenerated the card outputs" in c17b.log and "Another line a release adds" in (c17b.live("etc/pipeos/mandate.md") or "") and real_verify(c17b) == 0,
      "rc=%d verify=%d log=%r" % (c17b.rc, real_verify(c17b), c17b.log[-400:]))
