...
# ── 17c. a box claimed through the wizard has an EMPTY NICK (a GENERIC box) and the
# provisioned marker; a template deploy must regenerate there too (pipeOS#313: zero
# read CRITICAL after two template deploys on 2026-09-19 UTC because this step took an
# empty NICK for "unprovisioned")
c17c = case(fixture=REAL_FIXTURE, card=REAL_CARD.replace("NICK=box0", "NICK=").replace("NICK=\"box0\"", "NICK="))
write(os.path.join(c17c.root, "etc/pipeos/provisioned"), "")
g = subprocess.run(["sh", PBC, "generate", "--card", os.path.join(c17c.root, "etc/pipeos/card.conf"),
                    "--root", c17c.root, "--templates", TMPL_DIR], capture_output=True, text=True)
assert g.returncode == 0, "row 17b setup: generate failed: " + g.stderr
tp = os.path.join(c17c.repo, "overlay/usr/local/share/pipeos/card/mandate.md.tmpl")
write(tp, open(tp).read() + "\nAnother line a release adds.\n")
git(c17c.repo, "add", "-A"); git(c17c.repo, "commit", "-qm", "template change")
c17c.run("--dry-run")
dry_said_c = "would regenerate" in c17c.log
c17c.run()
check("17c a claimed box with an empty NICK (GENERIC, claimed through the wizard: the provisioned marker) has its card outputs regenerated by a template deploy too — and the dry run says so — and verify is clean after (pipeOS#313)",
