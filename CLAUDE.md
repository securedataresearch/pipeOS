...
`docs/fleet-ops.md` is the reference; `.claude/skills/pipeos-fleet` is the
skill. Every change to a live box goes through a `pipeos` verb (`schedule`,
`usage cap`, `card set`, `secrets phrase`, `assistant password`, `vault`,
`deploy-overlay`, …) — never a hand write over ssh. A task with no verb
gets a verb and a row in `scripts/check-operator-verbs.py`.
