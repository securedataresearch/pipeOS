...
Provisioning a box, end to end: build image (`make image VARIANT=usb`, card +
root key baked into `overlay/`) → `flash-box.sh` → boot → `pipebox-setup`
(card-generate + sign-in + save). Bugs that used to break this — clean-checkout
